...
export const validateEditProfileData = (req: Request)=>{
  // const allowedEdits = ["firstName", "lastName", "password", "age", "photoUrl", "about", "skills", "image"];
  // const isEditAllowed = Object.keys(req.body).every((edit) => allowedEdits.includes(edit));
  // if (!isEditAllowed || !req.file) {
  //   throw new Error("Invalid Edit");
  // }
  if(req.body.email){
    throw new Error("Email cannot be changed");
  }
  const {firstName, lastName, password, age, about, skills} = req.body;
